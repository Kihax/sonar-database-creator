...
# =====================================================================
# CONFIGURATION DES AUGMENTATIONS (Paramètres à ajuster)
# =====================================================================
# Définis ici les variations (translations en pixels/pings et rotations) souhaitées.
# Note : L'imagette de base (sans offset/rotation) est extraite automatiquement en premier.
...
# =====================================================================
dci = DatabaseCreatorImagette("./sensitivity-6areas-100-4t-300-30.nc")
init_main = False
...
for idx, target_point in enumerate(points_interest):
    ...
    print(f"\nRecherche et génération pour le Point {idx+1}/{total_groups} : indices ({p_idx}, {s_idx})")
    
    # Appel de la nouvelle fonction pour extraire l'imagette d'origine + les augmentations définies
    ...
    if imagettes_valides:
        print(f"-> {len(imagettes_valides)} imagette(s) générée(s) (Originale + {len(imagettes_valides)-1} augmentations)")
...
dci.close()
print("\n🎉 Nouvelle base d'intérêt avec augmentations générée avec succès.")