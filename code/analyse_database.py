...
dt = get_tree_from_file("Grid-All-eq-sf100.nc", "../")
dts = [dt]
...
# 3. Récupération des données (Detection Range) et filtrage (< 0.15)
...
print(f"Nombre d'imagettes proches du Nadir (> 0.4) : {nadir_count} ({(nadir_count / total_imagettes * 100) if total_imagettes else 0:.1f}%)")
...
if detection_ranges:
    ...
    near_nadir_pct = (
        float(np.sum(detection_ranges_arr > 0.4)) / len(detection_ranges_arr)
    ) * 100.0
