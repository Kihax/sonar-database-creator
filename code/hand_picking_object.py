...
for i, filename in enumerate(files):
    ...
    for obj in objs:
        ...
        objs_global_for_plot.append([x, y_global])

    # Conversion des listes de trajectoire en tableaux numpy
    ship_x = np.array(ship_x)
    ship_y = np.array(ship_y)
    ship_headings = np.array(ship_headings)

    # ========================================================
    # 🖼️ FENÊTRE 1 : Waterfall + Détections (Pixels)
    # ========================================================
    fig1, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))
    objs_global_for_plot = np.array(objs_global_for_plot)
    
    ax1.set_title("Bounding Boxes (Your Exact Morph + RegionProps)")
    ax1.imshow(image_bounding_boxes, aspect="auto", extent=img_extent_corr)
    if len(objs_global_for_plot) > 0:
        ax1.scatter(objs_global_for_plot[:, 0], objs_global_for_plot[:, 1], s=10, color='red', label="Centroids")
    ...
    if len(obj) > 0:
        ax3.scatter(objs_geo_x, objs_geo_y, color='lime', edgecolors='black', 
                    marker='^', s=40, label="Detected Objects", zorder=4)

        
    # 2. Ajout de flèches de cap (tous les X pings pour ne pas surcharger la figure)
    ...
    ax3.scatter(ship_x[0], ship_y[0], color='green', marker='o', s=100, label="Start", zorder=5)
    ax3.scatter(ship_x[-1], ship_y[-1], color='darkred', marker='X', s=100, label="End", zorder=5)
