...
class SonarPairDataset(Dataset):
    ...
    def _prepare_imagette(self, img_obj):
        ...
        if(self.meta_config.get("channel", None) is not None):
            ...
            for ch in self.meta_config.get("channel", []):
                if ch == "image":
                    ...
                    channels.append(pitch_2d)
                elif ch == "roll":
                    channels.append(roll_2d)
                elif ch == "heading":
                    channels.append(heading_2d)
                elif ch == "heading_sin":
        ...
        roll_norm = float(self.meta_config.get("roll_normalization", 360.0))
        pitch_norm = float(self.meta_config.get("pitch_normalization", 360.0))
        heading_norm = float(self.meta_config.get("heading_normalization", 360.0))
        ...
        if depth_val is not None:
            ...
            mean_depth = 0.0
        mean_depth = np.clip(mean_depth / depth_norm, 0.0, 1.0)
        ...
        if(self.meta_config.get("meta", None) is not None):
            meta_features = []
            for meta_name in self.meta_config.get("meta", []):
                if meta_name == "depth":
                    meta_features.append(mean_depth)
                elif meta_name == "roll":
                    meta_features.append(mean_roll)
                elif meta_name == "pitch":
                    meta_features.append(float(np.mean(pitch_vec)) / pitch_norm if len(pitch_vec) > 0 else 0.0)
                elif meta_name == "heading":
                    meta_features.append(mean_heading / heading_norm)
                elif meta_name == "heading_sin":
                    meta_features.append(np.sin(mean_heading_rad))
                elif meta_name == "heading_cos":
                    meta_features.append(np.cos(mean_heading_rad))
                elif meta_name == "detection_range":
                    meta_features.append(det_range)
                elif meta_name == "centered":
...
def prepare_datasets(config: dict):
    ...
    min_detection_range = float(config.get("min_detection_range", 0.0))
    max_detection_range = float(config.get("max_detection_range", 1.0))
    centered_filter = config.get("centered", None)

    def _filter_images(images):
        filtered = []
        for img in images:
            det_range = getattr(img, "detection_range", 0.0)
            if det_range is None:
                det_range = 0.0
            if det_range < min_detection_range or det_range > max_detection_range:
                continue
            filtered.append(img)
        return filtered
    ...
    # Filtrage des zones
    matching_groups = {}
    for coord, images in global_imagettes.items():
        images_filtered = _filter_images(images)
        if len(images_filtered) > min_imagettes:
            matching_groups[coord] = images_filtered

    imagettes_conservees = sum(len(images) for images in matching_groups.values())
    print(f"Zones conservées (> {min_imagettes} imagettes après filtrage) : {len(matching_groups)} / {len(global_imagettes)}")
    print(f"Total d'imagettes conservées après filtrage : {imagettes_conservees}")
